...
int main(int argc, char *argv[])
{
	...
	if (is_square){ //if square
		if (is_symmetric){
			...
			if (low_bandwidth < r2){
				...
				#pragma omp parallel
				{
					...
					for (int j = 0; j < low_bandwidth; j++){
						int curr_degree = (int) round ( norm_box_muller(avg, std, random_seed*(j+10)));
						...
						unsigned int mystate = random_seed * (j+5) + curr_degree;
						...
						for (int k = j+1; k < j+low_bandwidth; k++) { // after diagonal
							if (is_nz_ind [k]){
								indices[id_second - j][curr_degree++] = k + rlj;
							}
						}
						indices[id_second - j][curr_degree++] = id_second - j ; // make diagonal nz
						degrees[id_second - j] = curr_degree;
					}
			...
			else{  // low_bandwidth > r2
				
				#pragma omp parallel
				{
					USHI *is_nz_ind = (USHI*) safe_malloc(id_second * sizeof(USHI));
					#pragma omp for
					for (int j = 0; j < row_cnt - low_bandwidth; j++){
						int curr_degree = (int) round ( norm_box_muller(avg, std, random_seed*(j+10)));
					...
					#pragma omp for
					for (int j = row_cnt - low_bandwidth; j < r2; j++){
						int curr_degree = (int) round ( norm_box_muller(avg, std, random_seed*(j+10)));
						...
						//randomly fill nonzero values
						for (int k = 0; k < curr_degree-1; k++){
							is_nz_ind [rand_r(&mystate) % row_cnt] = 1;
						}
						...
						curr_degree = 0;
						for (int k = j+1; k < row_cnt; k++) { // after diagonal
							if (is_nz_ind [k]){
								indices[id_second - j][curr_degree++] = k + rlj;
							}
						}
			...
			if ( low_bandwidth > ru ){// first case for bandwidth
				#pragma omp parallel
				{
					USHI *is_nz_ind = (USHI*) safe_malloc(id_second * sizeof(USHI));
					#pragma omp for
					...
					#pragma omp for
					for (int j = low_bandwidth; j < id_second; j++){
						int curr_degree = (int) round ( norm_box_muller(avg, std, random_seed*(j+10)));
			...
			else{ // second case for bandwidth
...
void printusage()
{
	...
	printf("\t-u up_bandwidth : upper matrix bandwidth (valid if matrix is square)\n");
